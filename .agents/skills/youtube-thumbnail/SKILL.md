...
**However, do ask about specific visual elements.** Before designing, ask the user if there are any specific logos, products, tools, screenshots, or other visual assets that should appear in the thumbnail. For example: "Should I include any specific logos (Codex, Cursor, etc.) or product shots?" This takes 5 seconds and avoids wasting a generation on the wrong references.

Pick the first available headshot from `.Codex/skills/youtube-thumbnail/assets/headshots/`. If the folder is empty, tell the user to add a headshot photo there first.
...
```bash
python3 .Codex/skills/youtube-thumbnail/scripts/search_examples.py \
  --query "{video topic}" \
  --top 5 \
  --min-views 10000
```
...
**What to fetch:**
- **Tool/product logos** — If the video is about Codex, Cursor, Make.com, OpenAI, etc., fetch their official logos or icons
- **Mascots or brand visuals** — e.g., the Anthropic Codex logo, the OpenAI logo mark, app icons from the App Store
...
**How to fetch:**
1. Use `WebSearch` to find the best image URL (e.g., search for "Codex logo PNG transparent" or "Cursor IDE icon")
2. Use `Bash` with `curl` to download AND validate the image in one step:
   ```bash
   mkdir -p workspace/refs && \
   curl -sL "https://example.com/logo.png" -o "workspace/refs/Codex-logo.png" && \
   file workspace/refs/Codex-logo.png
   ```
...
```bash
python3 .Codex/skills/youtube-thumbnail/scripts/generate_thumbnail.py \
  --headshot ".Codex/skills/youtube-thumbnail/assets/headshots/{selected-headshot}" \
...
- In the prompt, explicitly tell Gemini which attached reference image is which: "The second attached image is the Codex logo — place it in the upper-left" or "The third attached image is a screenshot of the Cursor IDE — use it as the dashboard element on the left side"
...
```bash
python3 .Codex/skills/youtube-thumbnail/scripts/combine_thumbnails.py \
...
Examples:
- "Use the Codex logo from Image 2, floating in the upper-left with a subtle shadow"
...
```
- Image 2 (Codex logo): The official Codex terminal icon. Use this as the app icon in the upper-left area.
