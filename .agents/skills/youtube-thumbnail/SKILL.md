...
Scripts are shared from the full youtube pipeline skill at `.Codex/skills/youtube/scripts/` — no duplication.
...
```bash
python3 .Codex/skills/youtube/scripts/generate_thumbnail.py \
  --headshot "$HEADSHOT" \
...
```bash
python3 .Codex/skills/youtube/scripts/combine_thumbnails.py \
  --images \
    "$OUTPUT_DIR/a.png" \
    "$OUTPUT_DIR/b.png" \
    "$OUTPUT_DIR/c.png" \
...
| Script not found         | Wrong working directory               | Run from repo root (where `.Codex/` lives)  |
